...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def slowCompute(self, state, action):
        ...
        parts = [ prob * (self.mdp.getReward(state, action, sp) + (self.discount * self.values[sp])) 
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        return sum([ prob * (self.mdp.getReward(state, action, sp) + (self.discount * self.values[sp])) 
                for sp, prob in self.mdp.getTransitionStatesAndProbs(state, action)]
                )
